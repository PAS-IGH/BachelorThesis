...
def getAnomalyThreshold(df_series_ano, df_series_forecast, n_jarque_bera = None):
    # maybe average them as they are the mean anyway?
    return getMADScores(df_series_ano, df_series_forecast, n_jarque_bera).max()
...
def getMADScores(df_series_ano, df_series_forecast, n_jarque_bera = None): 
    ...
    if not n_jarque_bera:
        df_err = df_series_ano - df_series_forecast
